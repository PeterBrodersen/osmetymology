...
{
    global $dbh;
    $dbh->query('DROP TABLE IF EXISTS wikidata');
    $dbh->query('
    ...
    $dbh->query('CREATE UNIQUE INDEX wikidata_itemid_idx ON wikidata ("itemid")');
    $dbh->query('CREATE INDEX wikidata_name_idx ON wikidata ("name")');

    $dbh->query('DROP TABLE IF EXISTS wikilabels');
    $dbh->query('
        CREATE TABLE wikilabels (
            id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            itemId TEXT,
    ...
    ');
    $dbh->query('CREATE INDEX wikilabels_label_idx ON wikilabels ("label" text_pattern_ops)');
    $dbh->query('CREATE INDEX wikilabels_searchlabel_idx ON wikilabels ("searchlabel" text_pattern_ops)');
    return TRUE;
}
...
function getMissingItemIds()
{
    ...
    $tableExists = $dbh->query("
        SELECT to_regclass('wikidata') IS NOT NULL AS exists
    ...
    $itemIds = $dbh->query(
        ...
        LEFT JOIN wikidata ON single_items = wikidata.itemid
...
function getInstanceOfItems()
{
    ...
    $instanceofItems = $dbh->query(
        ...
        AND claims->'P31'->0->'mainsnak'->'datavalue'->'value'->>'id' NOT IN (SELECT itemid FROM wikidata)
        EOD,
        PDO::FETCH_COLUMN,
        0
    )->fetchAll();
...
function importItemIds($itemIds)
{
    ...
    $dbh->query("DELETE FROM wikidata WHERE itemId IN ('" . implode("','", $itemIds) . "')");
...
{
    global $dbh;
    $dbh->query('CREATE INDEX wikidata_claims_gin_idx ON wikidata USING gin(claims)');
    return true;
}
...
function insertLabels()
{
    ...
    $dbh->query(
        ...
        (
        SELECT itemid, name AS label, toSearchString(name) as searchlabel 
        FROM wikidata
        UNION
