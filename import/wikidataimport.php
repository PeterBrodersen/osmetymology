...
$insertdb = $dbh->prepare('
    INSERT INTO ' . DBSCHEMA . '.wikidata (itemid, name, description, labels, descriptions, claims, sitelinks, aliases)
    VALUES (?,?,?,?,?,?,?,?)
');
...
{
    global $dbh;
    $dbh->query('DROP TABLE IF EXISTS ' . DBSCHEMA . '.wikidata');
    $dbh->query('
        CREATE TABLE ' . DBSCHEMA . '.wikidata (
    ...
    $dbh->query('CREATE UNIQUE INDEX wikidata_itemid_idx ON ' . DBSCHEMA . '.wikidata ("itemid")');
    $dbh->query('CREATE INDEX wikidata_name_idx ON ' . DBSCHEMA . '.wikidata ("name")');

    $dbh->query('DROP TABLE IF EXISTS ' . DBSCHEMA . '.wikilabels');
    $dbh->query('
        CREATE TABLE ' . DBSCHEMA . '.wikilabels (
            id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            itemId TEXT,
    ...
    ');
    $dbh->query('CREATE INDEX wikilabels_label_idx ON ' . DBSCHEMA . '.wikilabels ("label" text_pattern_ops)');
    $dbh->query('CREATE INDEX wikilabels_searchlabel_idx ON ' . DBSCHEMA . '.wikilabels ("searchlabel" text_pattern_ops)');
    return TRUE;
}
...
function getMissingItemIds()
{
    ...
    $tableExists = $dbh->query("
        SELECT to_regclass('" . DBSCHEMA . ".wikidata') IS NOT NULL AS exists
    ")->fetch(PDO::FETCH_ASSOC);
    if (!$tableExists || !$tableExists['exists']) {
        die("Error: Table " . DBSCHEMA . ".wikidata does not exist." . PHP_EOL);
    }
    $itemIds = $dbh->query(
        <<<EOD
        WITH split_content AS (
        ...
        LEFT JOIN $dbschema.wikidata ON single_items = wikidata.itemid
...
function getInstanceOfItems()
{
    ...
    $instanceofItems = $dbh->query(
        ...
        AND claims->'P31'->0->'mainsnak'->'datavalue'->'value'->>'id' NOT IN (SELECT itemid FROM $dbschema.wikidata)
        EOD,
        PDO::FETCH_COLUMN,
        0
    )->fetchAll();
...
function importItemIds($itemIds)
{
    ...
    $dbh->query("DELETE FROM " . DBSCHEMA . ".wikidata WHERE itemId IN ('" . implode("','", $itemIds) . "')");
...
{
    global $dbh;
    $dbh->query('CREATE INDEX wikidata_claims_gin_idx ON ' . DBSCHEMA . '.wikidata USING gin(claims)');
    return true;
}
...
function insertLabels()
{
    global $dbh;
    $dbschema = DBSCHEMA; // for use in query string; can't be used directly in heredoc
    $dbh->query('TRUNCATE ' . DBSCHEMA . '.wikilabels'); // Truncate table before inserting
    $dbh->query(
        <<<EOD
        INSERT INTO $dbschema.wikilabels (itemId, label, searchlabel)
        (
        SELECT itemid, name AS label, $dbschema.toSearchString(name) as searchlabel 
        FROM $dbschema.wikidata
        UNION
        SELECT itemid, value->>'value' AS label, $dbschema.toSearchString(value->>'value') as searchlabel
        FROM $dbschema.wikidata, jsonb_array_elements(aliases->'da')
        )
        EOD
    );
}
...
function handlePropertyItems($property)
{
    ...
    $query = "
    WITH items AS (
        SELECT DISTINCT jsonb_array_elements(claims->'$property')->'mainsnak'->'datavalue'->'value'->>'id' AS id
        FROM " . DBSCHEMA . ".wikidata
    )
