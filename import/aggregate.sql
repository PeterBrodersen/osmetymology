...
(
	SELECT op.name, toSearchString(op.name), 'point', featureType(jsonb_merge_agg(tags)), a.area_id, array_agg(op.node_id), "name:etymology", "name:etymology:wikipedia","name:etymology:wikidata", regexp_split_to_array("name:etymology:wikidata", '\s*;\s*'), ST_Intersection(ST_Transform(ST_Collect(geom), 4326), a.wkb_geometry)
	FROM osm_points op
...
UNION
(
	SELECT ow.name, toSearchString(ow.name), 'line', featureType(jsonb_merge_agg(tags)), a.area_id, array_agg(ow.way_id), "name:etymology", "name:etymology:wikipedia","name:etymology:wikidata", regexp_split_to_array("name:etymology:wikidata", '\s*;\s*'), ST_Intersection(ST_Transform(ST_Collect(geom), 4326), a.wkb_geometry)
	FROM osm_ways ow
...
UNION (
	SELECT op.name, toSearchString(op.name), 'polygon', featureType(jsonb_merge_agg(tags)), a.area_id, array_agg(op.area_id), "name:etymology", "name:etymology:wikipedia","name:etymology:wikidata", regexp_split_to_array("name:etymology:wikidata", '\s*;\s*'), ST_Intersection(ST_Transform(ST_Union(geom), 4326), a.wkb_geometry)
