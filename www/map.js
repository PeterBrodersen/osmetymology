...
function formatDistanceAway(distanceInMeters, unitSystem = 'metric') {
    ...
    if (unitSystem === 'imperial') {
        ...
        if (distanceInMiles < 1) {
            ...
            return `${roundedFeet.toLocaleString()} ${roundedFeet === 1 ? 'fod' : 'fod'} væk`;
        }

        if (distanceInMiles > 100) {
            const roundedMiles = Math.round(distanceInMiles);
            return `${roundedMiles.toLocaleString()} ${roundedMiles === 1 ? 'mil' : 'mil'} væk`;
        }

        const roundedMiles = Math.round(distanceInMiles * 10) / 10;
        return `${roundedMiles.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })} mil væk`;
    }

    if (distanceInMeters < 1000) {
        const roundedMeters = Math.round(distanceInMeters);
        return `${roundedMeters.toLocaleString()} ${roundedMeters === 1 ? 'meter' : 'meter'} væk`;
    }

    const distanceInKilometers = distanceInMeters / 1000;
    if (distanceInKilometers > 100) {
        const roundedKilometers = Math.round(distanceInKilometers);
        return `${roundedKilometers.toLocaleString()} ${roundedKilometers === 1 ? 'kilometer' : 'kilometer'} væk`;
    }

    const roundedKilometers = Math.round(distanceInKilometers * 10) / 10;
    return `${roundedKilometers.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kilometer væk`;
}
